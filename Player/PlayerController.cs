...
{
    Rigidbody2D rbody; // Ringidbody2D型の変数　
    float axisH = 0.0f; //入力
    ...
    void Update()
    {
        ...
        //スペースキーがおされたかどうか　キャラクターをジャンプさせる
    ...
    void FixedUpdate()
    {
        ...
        //地上判定
        //円のセンサーを設置して、指定した特定のレイヤーに引っかかればtrue
        bool onGround = Physics2D.CircleCast(
       transform.position, //発動位置→プレイヤーの位置（足元)
       0.2f, //円の半径
       Vector2.down, //発動方向＜new Vector2(0.-1)>=Vector2.down
       0.0f,  //発射距離
       groundLayer //検出するレイヤー
       );
        //地面にいる時はvelocityが全てに反応
        ...
        {
            //地面の上でジャンプキーが押された
            //後のAddForceメゾットの第一引数にいれる方向のデータをあらかじめ作っておく　ジャンプさせる
        ...
        if (onGround)//地上の上なら
        {
            ...
            if (axisH == 0)
            {
                nowAnime = stopAnime; //停止中　「PlayerStop」クリップに更新すべき
            }
            else
            {
                nowAnime = moveAnime; //移動　「PlayerMove」クリップに更新すべき
            }
        }
        else　//空中なら
        {
