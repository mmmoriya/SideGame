...
public class MovingBlock : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isCanMove)
        {
            ...
            if (movep >= 1.0f)
            {
                ...
                {
                    //乗った時に動くフラグOFF 時間差で再始動(Moveメソッドをwait秒後に発動）
                    Invoke("Move", wait); //
                }
            }
        }
    }
    //移動フラグを立てる 止まっているフラグをtrueに戻すメソッド
    public void Move()
    {
        isCanMove = true;
    }

    //移動フラグを下ろす 動いているフラグをfalseにして止めるメソッド
    public void Stop()
    {
        isCanMove = false;
    }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        //移動の線を描画
        Gizmos.DrawLine(fromPos, new Vector2(fromPos.x + moveX, fromPos.y + moveY)); //スプライトのサイズ
